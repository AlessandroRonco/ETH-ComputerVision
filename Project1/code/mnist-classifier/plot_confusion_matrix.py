...
if __name__ == '__main__':
    ...
    # Create the network.
    net = MLPClassifier()
    # try different Classifiers uncommenting the following line
    # net = ConvClassifier()
    ...
    for batch in valid_dataloader:
        ...
        prediction_vector = torch.argmax(output, dim=1)
        annotation_vector = batch["annotation"]
        for i in range(0, BATCH_SIZE):
            prediction = prediction_vector[i]
            true_value = annotation_vector[i]
            if prediction == true_value:
                confusion_matrix[prediction, prediction] += 1
            else:
                confusion_matrix[prediction, true_value] += 1
